...
import { createUserWithEmailAndPassword, sendEmailVerification, GoogleAuthProvider, signInWithPopup, FirebaseError } from 'firebase/auth';
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = error instanceof FirebaseError ? error.message : "فشل إنشاء الحساب";
      toast({
        variant: "destructive",
    ...
    <div className="min-h-screen flex items-center justify-center bg-muted/30 p-4" dir="rtl">
      <Card className="w-full max-w-md border-0 shadow-xl rounded-[2.5rem] overflow-hidden">
        <CardHeader className="space-y-1 text-center pb-8 pt-10">
          <CardTitle className="text-3xl font-black text-primary">إنشاء حساب جديد</CardTitle>
          <CardDescription className="text-muted-foreground font-medium">ابدأ رحلتك مع مدمك هوم اليوم</CardDescription>
        </CardHeader>
        ...
        <CardFooter className="flex flex-wrap items-center justify-center gap-2 pb-10 bg-muted/20">
          ...
          <Link href="/login" className="text-sm font-black text-primary hover:underline">
            تسجيل الدخول
          </Link>
        </CardFooter>
      </Card>
